//
//  OrangeZJAnimationPopView.m
//  OrangeZJAnimationPopView <https://github.com/Abnerzj/OrangeZJAnimationPopView>
//
...
@interface OrangeZJAnimationPopView ()<UIGestureRecognizerDelegate>
...
@implementation OrangeZJAnimationPopView

+ (instancetype)share_CustomNativeByte_Manager {
    static OrangeZJAnimationPopView *popView = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        popView = [[OrangeZJAnimationPopView alloc] init];
    });
    return popView;
}

- (nullable instancetype)initWith_CustomNativeByte_CustomView:(UIView *_Nonnull)customView
...
//    }
    ...
    if (self) {
        ...
        _isObserver_CustomNativeByte_OrientationChange = NO;
        _popBGAlpha = 0.5f;
        _isTransparent = NO;
        _customView = customView;
        _animationPopStyle = popStyle;
        _animationDismissStyle = dismissStyle;
        _popAnimationDuration = 0;
...
- (void)setisObserver_CustomNativeByte_OrientationChange:(BOOL)isObserver_CustomNativeByte_OrientationChange
{
    _isObserver_CustomNativeByte_OrientationChange = isObserver_CustomNativeByte_OrientationChange;
    
    if (_isObserver_CustomNativeByte_OrientationChange) {
...
- (void)dismiss
{
    ...
    if (ws.isObserver_CustomNativeByte_OrientationChange) {
