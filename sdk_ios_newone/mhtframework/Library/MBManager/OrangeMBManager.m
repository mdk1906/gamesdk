...
#pragma mark - 简短提示语
+ (void) show_CustomNativeByte_BriefAlert:(NSString *) message inView:(UIView *) view {
    [self show_CustomNativeByte_BriefAlert:message time:kShowTime inView:view isHerizotal:NO];
}
+ (void)show_CustomNativeByte_BriefAlert:(NSString *)message time:(NSInteger)showTime inView:(UIView *)view {
    [self show_CustomNativeByte_BriefAlert:message time:showTime inView:view isHerizotal:YES];
}
+ (void)show_CustomNativeByte_BriefAlert:(NSString *)message time:(NSInteger)showTime inView:(UIView *)view isHerizotal:(BOOL)isHerizontal {
    dispatch_async(dispatch_get_main_queue(), ^{
        prestrainView = view;
        OrangeMBProgressHUD *hud = [OrangeMBProgressHUD showHUDAddedTo:view ?:kDefaultView animated:YES];
...
//        [hud hide:NO afterDelay:showTime];
        ...
        if (isHerizontal) {
//            hud.transform = CGAffineTransformMakeRotation(M_PI_4);
        }
    });
}
#pragma mark - 长时间的提示语
+ (void) showPermanentMessage:(NSString *)message inView:(UIView *) view{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        kDefaultRect;
        OrangeMBProgressHUD *hud = [OrangeMBProgressHUD showHUDAddedTo:gloomyView animated:YES];
...
+ (void) showLoadingInView:(UIView *) view{
    dispatch_async(dispatch_get_main_queue(), ^{
        prestrainView = view;
        OrangeMBProgressHUD *hud = [[OrangeMBProgressHUD alloc] initWithView:gloomyView];
...
+(void)show_CustomNativeByte_BriefAlert:(NSString *)alert{
    [self show_CustomNativeByte_BriefAlert:alert inView:nil];
}
...
+ (void)show_CustomNativeByte_BriefAlert:(NSString *)message time:(NSInteger)showTime {
    [self show_CustomNativeByte_BriefAlert:message time:showTime inView:nil isHerizotal:YES];
}
...
+(void)hideAlert{
    dispatch_async(dispatch_get_main_queue(), ^{
        OrangeMBProgressHUD *hud = [OrangeMBManager HUDForView:gloomyView];
#if 1
...
+ (OrangeMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:[OrangeMBProgressHUD class]]) {
            return (OrangeMBProgressHUD *)subview;
        }
    }
    return nil;
}
