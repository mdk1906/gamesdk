//
//  GreenFruitConfirmCodeInputViewBlackCustomControl.m
//  GreenFruitframework
//
...
@interface GreenFruitConfirmCodeInputViewBlackCustomControl()<XMTextFieldDelegate>
{
...
@end
@implementation GreenFruitConfirmCodeInputViewBlackCustomControl
...
-(void)configView{
    ...
    if ([self.jumpType isEqualToString:@"modifyPhone"]) {
        self.explanLabel.text = @"通过验证后可修改绑定手机号";
    }else{
        self.explanLabel.text = @"短信通过验证后即可重置密码";
...
-(void)initData{
    ...
    self.phoneNumberLabel.text = [NSString stringWithFormat:@"%@",self.showPhoneStr];
    
}

+ (instancetype)payCodeView
{
    GreenFruitConfirmCodeInputViewBlackCustomControl *payCodeView = [[GreenFruitConfirmCodeInputViewBlackCustomControl alloc]initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 65)];
    return payCodeView;
}
...
- (void)textFieldDidChange:(UITextField *)textField{
    ...
    if ([textField isEqual:_num1F]) {
        ...
        if ([self.jumpType isEqualToString:@"modifyPhone"]) {
            ...
            NSLog(@"验证手机号和验证码");
            [self checkPhoneAndCodeMethod:@"1"];//手机号
        }
    }
}
...
-(void)checkPhoneAndCodeMethod:(NSString *)style{
    [OrangeAPIParams requestCheckComfirmPhone:self.phone withConfirmCode:self.confirmCode withStyle:style SuccessBlock:^(id response) {
        ...
        if ([status isEqualToString:@"200"]) {
            ...
            if ([self.jumpType isEqualToString:@"modifyPhone"]) {
                if (self.modifyPhoneCheckBlock) {
                    self.modifyPhoneCheckBlock();
                }
            }
            else{
                [self jumpToSetPasswordView];
            }
...
-(void)jumpToSetPasswordView{
    
    if (self.checkSuccessBlock) {
        self.checkSuccessBlock(self.phone);
    }
}
...
- (IBAction)returnBackMethod:(id)sender {
    if ([self.jumpType isEqualToString:@"modifyPhone"]) {
        if (self.modifyphoneGobackBlock) {
            self.modifyphoneGobackBlock();
        }
...
- (IBAction)closeMethod:(id)sender {
    if ([self.jumpType isEqualToString:@"modifyPhone"]) {
        if (self.modifyphoneCloseviewBlock) {
            self.modifyphoneCloseviewBlock();
        }
